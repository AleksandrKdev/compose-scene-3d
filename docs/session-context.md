...
- Web camera projection now runs in the main vertex shader for both perspective and orthographic
...
Continue the final library milestones with universal node visibility/opacity on Android/iOS, then
API stabilization, documentation, and release preparation.
...
Compose screen anchoring for dimensions and arbitrary scene points, collision-free screen-label
...
focus, smooth cancellable camera flights, and gesture cancellation are done.
